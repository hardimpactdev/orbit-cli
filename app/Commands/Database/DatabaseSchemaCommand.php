...
final class DatabaseSchemaCommand extends GatewayCommand
{
    ...
    public function handle(): int
    {
        ...
        try {
            $response = $this->gatewayGet('/api/database-connections/schema', $this->filledQuery([
                'target' => $target,
        ...
        $this->line("Showing schema metadata for database connection '{$this->connectionSlug($response)}'.");
     ...
     */
    private function connectionSlug(array $response): string
    {
        $meta = $this->successMeta($response);
        $slug = $meta['connection'] ?? null;
