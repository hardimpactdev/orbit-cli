...
describe('GatewayStatus command', function (): void {
    ...
    it('renders the gateway status fields as flat key-value lines in human mode', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ...
            ->and($output)->toContain('version: 0.1.124')
            ->and($output)->toContain('time: 2026-06-18T00:00:00+00:00')
            ->and($output)->not->toContain('extra_top_level')
            ->and($output)->not->toContain('should-not-appear')
    ...
    it('renders flat status fields from an enveloped response without a nested gateway field', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ->and($output)->toContain('version: 0.0.0')
            ->and($output)->toContain('time: 2026-06-18T00:00:00Z')
            ->and($output)->not->toContain('success:')
            ->and($output)->not->toContain('{');
    });
